...
Making a custom distributed lag funciton.
...
test <- outcome_df_list %>% 
  map_dfr(~distributed_lag(., pm_df=3, temp_df=2, cumulative=T)) %>% 
  bind_cols(test, data_frame(outcome_names)) %>% 

test2 <- outcome_df_list %>% 
 

map_dfr

distributed_lag(asthma_df, pm_df=3, temp_df=2, cumulative=F)
