...
distributed_lag <- function(data, pm_df, temp_df, cumulative=T){
  ...
  # if degrees of freedom for pm is defined
  if(length(pm_df)==1){
  ...
  # if degrees of freedom is a vector > 1
  pm_df <- c(2:5)
  ...
  min_df_aic <- as.numeric(subset(pm_df_aic, mod_aic == min(mod_aic), 
                                  select=pm_df))
  # run final best fit mod
  pm_b <- splines::ns(0:(ncol(pm_matrix)-1), df=min_df_aic, intercept=T)
  # create pm basis
  pm_basis <- pm_matrix %*% pm_b
  # run model
  mod <- clogit(outcome ~ pm_basis + temp_basis + strata(personkey),
  ...
  # section to figure out model fit
  # 3 df
  # AIC(mod)
...
cumulative <- distributed_lag(hf65, pm_df = 3, temp_df = 2, cumulative = T)
cumulative
