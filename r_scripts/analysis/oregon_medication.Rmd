...
But I use GWR smoke of zip for the case crossover study and add the county variable.
...
# dataframe to loop through
inhaler_ndc_lag1_df <- data.frame(inhaler_ndc) %>%
...
# extract covariates from dataframe
covariates_df <- inhaler_ndc_lag1_df[, c(1:26, 74:79, 94:156)]
  
# extract pm values and divide by 10 and ordered
which(colnames(inhaler_ndc_lag1_df)=="geo_smk_pm_lag1_zip") # 136
pm_estimates_df <- inhaler_ndc_lag1_df[, 136]/10  # create 10 unit increases
...
# second loop to run a model for each pm estimation method
  ...
  # variable to model 
  var_name <- colnames(df_analysis[75])
  ...
  if(nrow(df_analysis) != 0){
  # conditional logistic regression model
  mod <- clogit(outcome ~ df_analysis[[75]] + wrf_temp_lag1_zip + strata(personkey), df_analysis)
