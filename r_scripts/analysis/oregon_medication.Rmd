...
## Overview
For this research I have the Oregon All Payer All Claims (APAC) data set in 2013. I want to research on the effects of wildfire smoke on pharmacy health outcome of asthma. NDC (National Drug Code) is the code that the data set offered. The standard NDC I use the link below, which is a ndc list in 2013, which fits my original data set. 
(http://ww2.ncqa.org/hedis-quality-measurement/hedis-measures/hedis-2013/hedis-2013-final-ndc-lists)

The data cleaning and casecrossover study are done with server, the following are done with local R.


## Preparation
1. Data cleaning

There are about 77,000,000 claims in total data set. Some have the NDC but another are null or missing. So I choose the claims in Oregon State with ndc, which is 23,330,714 claims. Also, the POS (Place of service code) all show "pharmacy" type.

For ndc, I choose the people who use the beta 2 agonists (one inhaler) for the day's first visit to form the sample. Because the ndc csv file has more information than what I need, I filter the ndc file by SQLite Studio (code and steps not attached) to a file only containing the beta 2 agonists medication. 

After choosing the claims with beta 2 agonists, the data set is reduced to 232,665. After choosing one day's first visit, the data set is the final data set we research on, containing 24,538 claims.

2. Smoke model

In this research, we foocus on GWR method (geographically weighted ridge regression). There are also WRF-Chem model (Weather Research and Forecasting with Chemistry) and Krig model. To reduce the occurance error I will compare all three models.
...
The time series for the number of pharmacy in each day (and separated with different counties).
The overall time series plot shows a 

!!! In claim data I make a proportion table of county and zip code because we need to make shapefiles. In this pharmacy I choose a different data set 
...
Because we could see a large and distinct strata dotted time series in some big counties (such as Multnomah), I hope to find if the weekday and weekend have the different observation numbers/
...
plot_ts_week <- ggplot(or_ndc_county_time_series_week, aes(y =n_obs, x = date)) +
           geom_jitter() + facet_wrap(~day_admit)

print(plot_ts_week)
ggsave("../plot_new/ndc_ts_week.pdf", plot = plot_ts_week, 
       width = 12, height = 8, units = "in")
...
plot_smk <- ggplot(or_ndc_county_time_series_week, aes(x = date)) +
  geom_jitter(aes(y =wrf_smk_pm, x = date), color='green', size = 2) + 
  geom_jitter(aes(y =krig_smk_pm, x = date), color='blue', size = 1) + 
  geom_jitter(aes(y =wrf_smk_pm, x = date), color="red", size = 1)  +
  ylab("Smoke PM 2.5") 
