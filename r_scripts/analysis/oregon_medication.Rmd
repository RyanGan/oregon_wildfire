...
We had previously shown GWR to be a good estimate of wildfire smoke concentrations, and showed associations with respiratory outcomes (including asthma) in Washington state. We want to replicate results for Oregon and also look at beta2 agonist pharmacy fills.
...
The data cleaning and creation of the casecross-over dataframes are huge computation tasks, so they are done with server, the following work is done with local R.
...
meta_ts <- read_csv("../data/meta_timeseries.csv") %>% 
  ...
  mutate(smoke0 = ifelse(geo_smk_pm > 0, 1, 0),
         ...
         # lag variable
         geo_smk_lag1 = lag(geo_smk10, 1, order_by = county),
         geo_smk_lag2 = lag(geo_smk10, 2, order_by = county),
         geo_smk_lag3 = lag(geo_smk10, 3, order_by = county),
         geo_smk_lag4 = lag(geo_smk10, 4, order_by = county),
         geo_smk_lag5 = lag(geo_smk10, 5, order_by = county),
         geo_smk_lag6 = lag(geo_smk10, 6, order_by = county),
         geo_smk_lag7 = lag(geo_smk10, 7, order_by = county)) %>% 
  ungroup()
...
These results show that there may be effect modification by sex, where females might be more likely to fill a beta-2 agonist on the same day it's really smoky for the GWR estimates.
...
Remove the missing values for GWR smoke. But I only use GWR smoke of zip for the case crossover study.
...
```{r DLM for casecrossover, echo = FALSE, message=FALSE, warning=FALSE}
inhaler_geo_lag <- inhaler_ndc_lag_df %>%
  select(159, 160:166) # GWR-method
...
```{r time stratified county for ts lag -7 to 0 prepare, echo=FALSE, include=FALSE}
read_path7 <- paste0("../data_new/medication/oregon_asthma_ndc.csv")
or_ndc_claim <- read_csv(read_path7)
...
```{r time stratified zip for casecrossover lag -7 to 0 df, echo=FALSE, include=FALSE}
...
```{r DLM for casecrossover lag -7 to 0, echo = FALSE, message=FALSE, warning=FALSE}
inhaler_geo_lag <- inhaler_ndc_lag_15_df %>%
  select(177:171, 178) # GWR-method
...
B <- ns(-7:lag_max, df =5, intercept=TRUE) 
