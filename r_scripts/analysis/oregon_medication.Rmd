...
state_n_obs <- or_ndc_county_time_series %>% 
  select(county, date, n_obs) %>% 
  gather(key = n_obs, value = n, -date, -county)
  

count_plot <- ggplot(state_n_obs, aes(x = date, y = n)) +
  geom_point() +
  facet_wrap(~county+n_obs, scales = "free") +
  ylab("Outcome Counts") +
  xlab("Date") +
  theme_bw()
...
ggsave("../plot_new/ndc_county_ts2.pdf", plot = count_plot, 
       width = 12, height = 8, units = "in")
...
a <- glm(n_obs ~ geo_smk_pm, data = or_ndc_county_time_series, family = "poisson")
summary(a)
...
rr_df <- data.frame(matrix(nrow = 3, ncol = 4))
colnames(rr_df) <- c("outcome", "rel_risk", "lower_95", "upper_95")

# vector of outcomes
outcome_list <- c("obs_n")
...
meow <- lapply(outcome_list, function(x){
 ...
 smk_est <- tidy(glm(as.formula(paste0(x, 
    "~ geo_smk10 + wrf_temp + county + 
    day + offset(log(population))")), 
    or_ndc_county_time_series, family="quasipoisson"))[2, ]
...
counties <- as.character(unique(or_ndc_county_time_series$county))

# create dataframe list of each county
county_list_df <- lapply(counties, function(x) 
  or_ndc_county_time_series[or_ndc_county_time_series$county==x,])
...
```{r overall lag 1, warning =F, echo = F, results='asis'} 
# dataframe list
...
```{r time stratified age strata lag 1, warning =F, echo = F, results='asis'} 
# dataframe list
...
```{r sex strata cdc met  lag 1, warning = F, echo = F, results='asis'} 
method_list <- c('WRF-Chem Smoke', 'Kriging Smoke', 'Geo-Weighted Smoke')
...
```{r overall lag 2, warning =F, echo = F, results='asis'} 
# dataframe list
...
```{r time stratified age strata lag 2, warning =F, echo = F, results='asis'} 
# dataframe list
...
```{r sex strata cdc met  lag 2, warning = F, echo = F, results='asis'} 
method_list <- c('WRF-Chem Smoke', 'Kriging Smoke', 'Geo-Weighted Smoke')
...
```{r overall lag 3, warning =F, echo = F, results='asis'} 
# dataframe list
...
```{r time stratified age strata lag 3, warning =F, echo = F, results='asis'} 
# dataframe list
...
```{r sex strata cdc met  lag 3, warning = F, echo = F, results='asis'} 
method_list <- c('WRF-Chem Smoke', 'Kriging Smoke', 'Geo-Weighted Smoke')
...
```{r overall lag 4, warning =F, echo = F, results='asis'} 
# dataframe list
...
```{r time stratified age strata lag 4, warning =F, echo = F, results='asis'} 
# dataframe list
...
```{r sex strata cdc met  lag 4, warning = F, echo = F, results='asis'} 
method_list <- c('WRF-Chem Smoke', 'Kriging Smoke', 'Geo-Weighted Smoke')
...
```{r overall lag 5, warning =F, echo = F, results='asis'} 
# dataframe list
...
```{r time stratified age strata lag 5, warning =F, echo = F, results='asis'} 
# dataframe list
...
```{r sex strata cdc met  lag 5, warning = F, echo = F, results='asis'} 
method_list <- c('WRF-Chem Smoke', 'Kriging Smoke', 'Geo-Weighted Smoke')
...
```{r overall lag 6, warning =F, echo = F, results='asis'} 
# dataframe list
...
```{r time stratified age strata lag 6, warning =F, echo = F, results='asis'} 
# dataframe list
...
```{r sex strata cdc met  lag 6, warning = F, echo = F, results='asis'} 
method_list <- c('WRF-Chem Smoke', 'Kriging Smoke', 'Geo-Weighted Smoke')
...
```{r overall lag 7, warning =F, echo = F, results='asis'} 
# dataframe list
...
```{r time stratified age strata lag 7, warning =F, echo = F, results='asis'} 
# dataframe list
...
```{r sex strata cdc met  lag 7, warning = F, echo = F, results='asis'} 
method_list <- c('WRF-Chem Smoke', 'Kriging Smoke', 'Geo-Weighted Smoke')
...
## NDC claims with index

Check the claims in each category (Asthma, COPD, Both or Other).

```{r echo = FALSE, message=FALSE, warning=FALSE}
read_path7 <- paste0("../data_new/medication/oregon_beta_ndc_index.csv")
or_ndc_index <- read_csv(read_path7)

oregon_beta_df <- or_ndc_index %>% 
  group_by(personkey) %>% 
  arrange(personkey, fromdate, line) %>% 
  mutate(num_visit = dense_rank(fromdate)) %>%
  arrange(personkey, num_visit) %>% 
  select(personkey, clmid, num_visit) %>% 
  unique() %>% 
  full_join(or_ndc_index, by = c("personkey", "clmid")) %>% 
  arrange(personkey, clmid, line, fromdate) %>% 
  filter(row_number() == 1) %>%
  # add new transverted from date
  mutate(dates = as.Date(fromdate, "%m/%d/%Y")) %>%
...
summary(as.factor(oregon_beta_df$index))
...
```{r ndc descriptive table, echo = F, message = F, warning = F, results='asis'}
read_path6 <- paste0("../data_new/medication/oregon_ndc_time_strat_casecrossover.csv")
beta_ndc <- read_csv(read_path6)

# join personkey with index
oregon_beta_index_df <- oregon_beta_df %>%
  select(personkey, index)

beta_ndc_with_index <- beta_ndc %>%
  left_join(oregon_beta_index_df, by="personkey")

# dataframe to loop through
beta_ndc_with_index_df <- data.frame(beta_ndc_with_index)

beta_asthma <- beta_ndc_with_index_df %>%
  filter(index=="asthma")

beta_copd <- beta_ndc_with_index_df %>%
  filter(index=="copd")

beta_both <- beta_ndc_with_index_df %>%
  filter(index=="both")

beta_other <- beta_ndc_with_index_df %>%
  filter(index=="other")


df_list <- list(beta_asthma, beta_copd, beta_both, beta_other)
...
# outcome name
outcome_name <- c("Asthma", "COPD", "Both", "Other")
...
```{r ndc overall try, warning =F, echo = F, results='asis'}
df_list <- list(beta_asthma, beta_copd, beta_both, beta_other)
...
for(i in 1:length(df_list)){
  ...
  # extract pm values and divide by 10 and ordered
  #which(colnames(df_to_loop)=="global_smk_pm_zip") # code to find column numbers
  pm_estimates_df <- df_to_loop[, c(87, 92, 91, 93)]/10  # create 10 unit increases
...
tab <- htmlTable(txtRound(table_df, digits = 3, 1:3), 
                 ...
                 # row group by outcome
                 rgroup = outcome_list,
                 n.rgroup = c(rep(3, 4)), # 4 rows for each method for each outcome
                 # column headers
...
print(print_plot)
# save figure
ggsave("../plot_new/fig_ndc_overall_four.pdf", plot = print_plot, 
       width = 12, height = 8, units = "in")
...
```{r ndc time stratified age strata, warning =F, echo = F, results='asis'} 
df_list <- list(beta_asthma, beta_copd, beta_both, beta_other)
...
## ggplot 
  ...
  print(print_plot)
  # save figure
  ggsave("../plot_new/ndc_age_fig_four.pdf", plot = print_plot, 
       width = 12, height = 8, units = "in")
...
```{r ndc sex strata, warning = F, echo = F, results='asis'} 
df_list <- list(beta_asthma, beta_copd, beta_both, beta_other)
...
## ggplot
  ...
  print(print_plot)
  # save figure
  ggsave("../plot_new/ndc_sex_fig_four.pdf", plot = print_plot, 
       width = 12, height = 8, units = "in")
...
oregon_beta_cat_df <- oregon_beta_df %>%
  select(personkey, num_visit, clmid, line, ZIP, dates, index) 


# create a working dataset 'or_2013'
oregon_county_beta_cat_ts <- oregon_beta_cat_df %>% 
  # join in the washington county name based on CHARS county identifier
  left_join(or_zip_county, by = "ZIP") %>% 
  # rename 'admit_date_impute' to 'date'
  rename(date = dates) %>%
...
# which(is.na(oregon_county_beta_cat_ts$county))
# oregon_county_beta_cat_ts[113,]
# which(or_zip_county$ZIP=="97078")

# summary(or_ndc_asthma)
# summary(oregon_county_beta_cat_ts)
summary(as.factor(oregon_county_beta_cat_ts$county)) # 125 is missing out of 3592 (3.16%)

# remove NA values
which(is.na(oregon_county_beta_cat_ts$county)) # 7728-7931
oregon_county_beta_cat_ts <- oregon_county_beta_cat_ts[-c(7728:7931),]

# write permanent dataframe

write_path5 <- paste0("../data_new/medication/or_county_beta_cat_ts.csv")
write_csv(oregon_county_beta_cat_ts, write_path5)

## Check for missing values in sparsely populated counties. set to 0 (But no)
# check_missing <- filter(oregon_county_beta_cat_ts, is.na(n_obs))
# mutate each to fill in missing values (Still no)
# check2 <- check_missing %>% mutate_each(funs(new = ifelse(is.na(.), 0, .)), 
#                                         n_obs:wrf_temp)

plot_ts <- ggplot(oregon_county_beta_cat_ts, aes(y =n_obs, x = date)) +
...
oregon_beta_rate <- oregon_county_beta_cat_ts %>%
  group_by(county, index) %>%
  summarise(population=n()) %>%  # 143 rows
  mutate
