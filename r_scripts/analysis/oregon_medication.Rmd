---
title: "The GWR Smoke's effect on Oregon 2013 Medication of Beta-2 agonists"
author: "Jingyang Liu"
date: "May 17, 2017"
...
2. Research question: We evaluated the association between smoke concentrations using the GWR method and SABA refills in Oregon state during the 2013 wildfire season.
...
3.Methods: use case-crossover for all health outcome figures and results. Make sure smoke is assigned via zipcode. For the descriptive of smoke, I think the county-level for now. We may move to zip.
...
or_smk_plot <- ggplot(or_smk, aes(x = date, y = pm)) +
  ...
  ylab(expression(paste("GWR Smoke PM"[2.5], " Concentration, µg/m^3" ))) +
...
smoke_map <- ggplot(or_smk_df, aes(x=long, y=lat, group=group)) +
  ...
  xlab("Longitude") +
  ylab("Latitude") +
  ggtitle("Oregon: Counties Affected by Smoke PM2.5 > 10µg/m^3") +
...
```{r input category, echo = FALSE, message=FALSE, warning=FALSE}
df_list <- list(asthma_df, copd_df, both_df, other_df)
...
# datalist_dml[[i]] <- data.frame(df_list[i]) %>%
  asthma_dml <- asthma_df %>%
  left_join(or_zip_to_county, by = "ZIPCODE") %>%
  left_join(or_pop, by = "county") %>%
  mutate(day = ifelse(weekdays(date) == "Monday" |
                      weekdays(date) == "Tuesday" |
...
asthma_dml$day[which(asthma_dml$date=="2013-05-27")] <- "weekend"
asthma_dml$day[which(asthma_dml$date=="2013-07-04")] <- "weekend"
asthma_dml$day[which(asthma_dml$date=="2013-09-02")] <- "weekend"


datalist_dml[[i]]$day[which(datalist_dml[[i]]$date=="2013-05-27")] <- "weekend"
datalist_dml[[i]]$day[which(datalist_dml[[i]]$date=="2013-07-04")] <- "weekend"
datalist_dml[[i]]$day[which(datalist_dml[[i]]$date=="2013-09-02")] <- "weekend"
}

for(i in 1:4){
  i <- 1

  inhaler_geo_lag <- asthma_dml %>%
  select(159, 160:166) # GWR-method
  
inhaler_temp_lag <- asthma_dml %>%
  select(93, 150:156) # GWR-method

dml_df <- datalist_dml[[i]]
