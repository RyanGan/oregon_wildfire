...
```{r regression model q, message=FALSE, echo=FALSE, eval=FALSE, warning=FALSE}
...
```{r compare mixed and fixed, message=FALSE, echo=FALSE, eval=FALSE}
...
### 2. Distributed lag for casecrossover
...
```{r DLM for casecrossover temp, echo = FALSE, message=FALSE, warning=FALSE}
read_path8 <- paste0("../data_new/county_data/zip_to_county_new.csv")
or_zip_to_county <- read_csv(read_path8)
...
### 3. Unconstrained lag for time series

```{r unconstrained ts lag, message=FALSE, echo=FALSE}
...
### 4. Distributed lag for time series
...
## mixed model
mod <-glmer(outcome ~  geo_ts_lag_B + tem_ts_lag_B + day + (1|county) + offset(log(pop)),   
