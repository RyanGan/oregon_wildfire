...
casecross <- function(data, id, admit_date, start_date, end_date, interval){
  ...
  id_date <- cbind(id_vec, admit_date)
  ...
  referent_date_list <- apply(id_date, 1, function(x){
    ...
    date_ref <- sort(unique(c(date_prior, date_posterior)))
    id_rep <- rep(x[1], length(date_ref))
    id_date_vec <- cbind(id_rep, date_ref)
    # outcome
    data_list$outcome <- data_list[data_list$date_vec]
    return(id_date_vec)
  }) # end apply
    # timestrat dataframe
  ts_data <- do.call(rbind, referent_date_list)
  
  # stuff below still needs to be figured out.
  
 
  

  names(referent_date_list) <- id_vec
  head(referent_date_list)
  test_list <- referent_date_list[1]

  casecross_data <- sapply(test_list, function(x){
    id_vec <- rep(names(x), lengths(x))
    date_vec <- unlist(x)
    cc_data <- cbind(id_vec, date_vec)
    return(cc_data)
  })
  
  do.call
  rep(names(test_list), lengths(test_list))
}
...
test <- casecross(data = test_ed, id="personkey", admit_date = "todate", 
                  start_date = "2013-05-01", end_date = "2013-10-1", interval = 7)
