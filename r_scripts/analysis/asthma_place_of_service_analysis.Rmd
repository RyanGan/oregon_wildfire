...
```{r saba_medford}
# subset medfore saba
saba_medford <- asthma_pos_list$saba %>% 
  filter(metroarea == "Medford") %>% 
  # remove missing values for lag
  filter(!is.na(geo_pm10_lag5))
```

Set up. I'm going to try a 3 knot ns spline for the lag funciton and a 2 knot spline for exposure response to PM~2.5~.
...
# crossbasis
cb_pm <- crossbasis(pm_lag_mat, lag = 5, argvar = list("ns", df = 2),
                    arglag = list(fun="ns", knots=c(2,4), intercept = T))

# clogit model
cb_mod <- clogit(outcome ~ cb_pm + strata(identifier), data = saba_medford) 

# predict
cb_pred <- crosspred(basis = cb_pm, model = cb_mod, cen = 6, at = seq(5,30,5))

```

Cumulative effect at different PM~2.5~ levels.

```{r}
# predictor variable
pm_levels <- cb_pred$predvar
# cumulative rel risk/odds ratio from clogit mod
odds_ratio <- cb_pred$allRRfit
odds_ratio

plot(pm_levels, odds_ratio)
```
