---
title: "Oregon GWR smoke association with Cardiopulmonary Emergency Room visits (PartA)"
author: "Jingyang Liu"
date: "April 9, 2018"
...
This project researches on for the association between GWR smoke PM~2.5~ and cardiopulmonary (cardiovascular (CVD) and respiratory health outcomes) diseases for Oregon State in wildfire season (May to September) in 2013. 
...
tab <- htmlTable(txtRound(table_df, digits = 3, 1:3), 
                 ...
                 rgroup = outcome_list,
                 n.rgroup = c(rep(1, 11)), # 4 rows for each method for each outcome
                 # column headers
...
wrf_geo_age <- combined_point_est_df %>% 
  ...
  tab <- htmlTable(txtRound(wrf_geo_age, digits = 3, 1:3), 
           ...
           # row group by outcome
           rgroup = outcome_list,
           n.rgroup = c(rep(3, 11)), # 6 rows for each age cat for each outcome
           # column headers
...
wrf_geo_sex <- combined_point_est_df %>% 
  ...
  tab <- htmlTable(txtRound(wrf_geo_sex, digits = 3, 1:3), 
           ...
           # row group by outcome
           rgroup = outcome_list,
           n.rgroup = c(rep(2, 11)), # 2 rows for each sex strata for each outcome
           # column headers
...
result_plot <- ggplot(results, aes(x = outcome, y = odds_ratio, color = Group)) +
  ...
  facet_wrap(~strata) +
  ylab(expression(paste("Odds Ratio for 10�g/m"^3, " Increase in PM"[2.5]))) +
  xlab("Outcome") +
  # plot theme
  theme(panel.background = element_rect(fill = 'white', colour = 'black'),
        panel.grid.major = element_blank(),
...
# print plot
ggsave("../plot_new/cardiopulmonary_er_result.tiff", plot = result_plot, units="in", width=12, height=8, dpi=400)

# ggsave("../plot_new/cardiopulmonary_er_result_table.pdf", plot = result_plot, 
#         width = 12, height = 8, units = "in", dpi = 400)

tiff('../plot_new/cardiopulmonary_er_result.tiff', units="in", width=12, height=8, res=300)
...
We see that all exposure methods are significantly associated with an increase in asthma with Geo-Weighted Regression, those are associated with COPD. I believe this is the most justifiable design approach that would average any extreme values of PM~2.5~ across the wildfire season for the vector of referent values for a particular subject.



*Note from Jingyang 04/12/2018:*  
Ryan said he is more likely interested in the diseases shown in plot (7 in total), not all cardiopulmonary diseases, however, I still loaded all the disease data set and reserved them in result and descriptive table to show the result.