...
# table ----
knitr::kable(personkey2, caption = "Patient ID")
...
state2 <- oregon_df %>%
  ...
  mutate(total_n = sum(n),
         percent = round(n/total_n, 4)*100,
         state_cat = ifelse(STATE == "OR", "Oregon",
                     ifelse(STATE != "OR"&is.na(STATE)==F, "Not Oregon",NA))) %>%
...
zip2 <- oregon_df %>%
  ...
  mutate(total_n = sum(n),
         percent = round(n/total_n, 4)*100,
         zip_cat = ifelse(is.na(ZIP) == 0, "Non-Missing",
                   ifelse(is.na(ZIP) != 0, "Missing", NA))) %>%
    select(ZIP, zip_cat, n, percent)
...
msa2 <- oregon_df %>%
  select(MSA) %>%
  mutate(msa_cat = ifelse(MSA != "", "Non-Missing",
...
pos2 <- oregon_df %>%
  ...
  mutate(total_n = sum(n),
         percent = round(n/total_n, 4)*100,
         Place_of_service = ifelse(pos == "20", "Urgent Care Facility",
                            ifelse(pos == "23", "Emergency Room – Hospital", 
                            ifelse(is.na(pos) == 0, "Not Urgent", 
                            ifelse(is.na(pos) ==1, "Missing", NA))))) %>%
...
fromdate2 <- oregon_df %>%
  select(fromdate) %>%
  mutate(dates = as.Date(fromdate2$fromdate, "%Y-%m-%d"))
...
todate2 <- oregon_df %>%
  select(todate) %>%
  mutate(ddd = as.Date(todate2$todate, "%Y-%m-%d"))


todate3 <- oregon_df %>%
  filter (todate > "2014-12-31")
todate3
...
hist(todate2$ddd, "days", freq = TRUE)
