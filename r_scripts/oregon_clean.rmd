...
for (i in 1: length(oregon_df$personkey)){
  visit$num_visit[i] <- length(unique(visit$clmid[which(visit$personkey==visit$personkey[i])]))
  
}

a<-unique(oregon_df$personkey)


visit2 <- visit %>%
  select(personkey, num_visit) %>%
  distinct(personkey, num_visit)
...
m <- mean(visit2$num_visit)
median(visit2$num_visit)
m # 4.61

knitr::kable(visit3, align = "c", caption = "Visits")

hist(visit3$num_visit, br= 20)

ggplot(visit3, aes(x=num_visit)) +
  geom_density()
