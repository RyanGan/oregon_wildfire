...
# import the full dataframe, change column class to character
read_path <- paste0("../data/gan_episodes_of_care.txt")
start_time <- Sys.time()
oregon_df <- fread(read_path, sep = "|", showProgress = T,fill = T, 
                   colClasses = rep("character", 72)) 
stop_time <-  Sys.time() - start_time 

# Note RG: fill = T allows fread to fill in any missing data
...
hist(visit3$num_visit, freq = T, xlab = "number of visit", main = "Visiting number for each personkey")
...
```{r}
pos_key <- read_excel("../data/LT_pos.xlsx")
...
## Total Time Markdown File Ran
```{r}
stop_time2 <-  Sys.time() - start_time 
stop_time2
```
