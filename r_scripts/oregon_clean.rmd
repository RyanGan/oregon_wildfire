...
personkey2 <- oregon_df %>% 
  select(personkey, gender) %>%
  filter(gender=="U") %>%
  mutate(id_ind =ifelse(is.na(personkey)==0 , 1, 
                        ifelse(is.na(personkey)==1, 0, NA))
   )

# table ----
knitr::kable(personkey2)

# preserve order of group
personkey2$group <- reorder(personkey2$id_ind, personkey2$personkey)
...
age2[4,1] <- age2[4,2] <- "Missing"

knitr::kable(age2, caption = "Age")
...
summary(as.factor(oregon_df$pos)) # 23
summary(as.factor(oregon_df$megtype)) # Acute for emergency?
