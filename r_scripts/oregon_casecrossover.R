...
### ---------------------------------------------------------------------
var_list <- c('resp', 'asthma', 'pneum',  'acute_bronch', 'copd', 'cvd', 
              'ihd', 'arrhythmia', 'hf', 'cereb_vas', 'mi', 'broken_arm')
...
one_asthma1_case <- sample_n(disease_asthma1, 1) %>% 
  select(personkey, id, fromdate, dates, dx1, ZIP, race, ethn, MSA, pos, sex_ind, age_ind)

one_asthma1_case

# find the replicate times of weeks
dates <- one_asthma1_case[[1,4]] 
n1 <- 0
